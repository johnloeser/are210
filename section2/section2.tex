...
\vspace{1em}
\noindent
3) \textbf{Mixed random variables}: Let $(X_{1}, X_{2})$ be mixed random variables with joint distribution $F = \alpha F^{C} + (1 - \alpha) F^{D}$, where $F^{C}$ is a continuous CDF and $F^{D}$ is a discrete CDF. How would you calculate the marginal distribution of $X_{1}$?
